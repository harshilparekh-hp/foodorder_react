...
function CartSummary() {
  ...
  const handleQuantity = (updateQuantityBy: number, cartItem: cartItemModel) => {
    if((updateQuantityBy == -1 && cartItem.quantity == 1) || updateQuantityBy == 0){
      // in this scenario, remove the entire item from the cart as the updateQuantityBy (remove button) = 0
      ...
      const response = updateShoppingCart({
        menuItemId: cartItem.menuItem?.id,
  ...
  return (
    <div className="container p-4 m-2">
      ...
      {shoppingCartFromStore && shoppingCartFromStore.length > 0 ? (
        ...
        <div className="text-center align-items-center">Shopping Cart Empty</div>
      )}
    </div>
  );
}
