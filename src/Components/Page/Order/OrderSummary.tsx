...
function OrderSummary({ apiResult, userInput }: OrderSummaryProps) {
  ...
  const badgeTypeColor = getStatusColor(apiResult.status!);
  ...
  // here, ! sign is for null check.
  const nextStatus: any =
    apiResult.status! === SD_Status.PENDING
      ? { color: "primary", value: SD_Status.CONFIRMED }
      : apiResult.status! === SD_Status.CONFIRMED
      ? { color: "info", value: SD_Status.BEGIN_COOKED }
      : apiResult.status! === SD_Status.BEGIN_COOKED
      ? { color: "light", value: SD_Status.READY_FOR_PICKUP }
      : apiResult.status! === SD_Status.READY_FOR_PICKUP && {
          color: "success",
      ...
      <div className="d-flex justify-content-between align-items-center mt-3">
        ...
        {userData.role == SD_Roles.ADMIN && (
          <div className="d-flex">
            {apiResult.status! !== SD_Status.CANCELLED &&
              apiResult.status! !== SD_Status.COMPLETED && (
